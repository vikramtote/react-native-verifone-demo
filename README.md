...
VerifoneNative3 Native Plugin for React Native for both Android (Classic and Native), iOS and Windows

Foundation of this library is based on Chris Brody's Cordova VerifoneNative plugin.
...
Features:
  ...
  5. Pre-populated VerifoneNative database import from application bundle and sandbox
...
Please let me know your projects that use these VerifoneNative React Native modules. I will list them in the reference section. If there are any features that you think would benefit this library please post them.
...
# Installation
```
  npm install --save react-native-verifone-demo
```
Then follow the instructions for your platform to link react-native-verifone-demo into your project

## Promises
To enable promises, run 
```javascript
VerifoneNative.enablePromise(true);
```
...
pod 'react-native-verifone-demo', :path => '../node_modules/react-native-verifone-demo'
```
...
##### Drag the VerifoneNative Xcode project as a dependency project into your React Native XCode project

![alt tag](https://raw.github.com/andpor/react-native-verifone-demo/master/instructions/libs.png)

##### XCode VerifoneNative libraries dependency set up

Add libVerifoneNative.a (from Workspace location) to the required Libraries and Frameworks. Also add sqlite3.0.tbd (XCode 7) or libsqlite3.0.dylib (XCode 6 and earlier) in the same fashion using Required Libraries view (Do not just add them manually as the build paths will not be properly set)

![alt tag](https://raw.github.com/andpor/react-native-verifone-demo/master/instructions/addlibs.png)

#### Step 2. Application JavaScript require

Add var VerifoneNative = require('react-native-verifone-demo') to your index.ios.js

![alt tag](instructions/require.png)

#### Step 3. Write application JavaScript code using the VerifoneNative plugin

Add JS application code to use VerifoneNative API in your index.ios.js etc. Here is some sample code. For full working example see test/index.ios.callback.js. Please note that Promise based API is now supported as well with full examples in the working React Native app under test/index.ios.promise.js
...
var db = VerifoneNative.openDatabase("test.db", "1.0", "Test Database", 200000, openCB, errorCB);
db.transaction((tx) => {
...
** React Native 0.60 and above **
If you would like to use the devices VerifoneNative there are no extra steps.
However, if you would like to use the VerifoneNative bundled with this library (includes support for FTS5), add the following to your `react-native.config.js`
  ...
  dependencies: {
    ...
    "react-native-verifone-demo": {
      platforms: {
        android: {
          sourceDir:
            "../node_modules/react-native-verifone-demo/platforms/android-native",
          packageImportPath: "import io.liteglue.VerifoneNativeModulePackage;",
          packageInstance: "new VerifoneNativeModulePackage()"
        }
      }
    }
    ...
...
include ':react-native-verifone-demo'
project(':react-native-verifone-demo').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-verifone-demo/platforms/android') // react-native-verifone-demo >= 4.0.0
// IMPORTANT: if you are working with a version less than 4.0.0 the project directory is '../node_modules/react-native-verifone-demo/src/android'
```
...
dependencies {
    ...
    implementation project(':react-native-verifone-demo')
}
```

#### Step 3 - Register React Package (this should work on React version but if it does not , try the ReactActivity based approach. Note: for version 3.0.0 and below you would have to pass in the instance of your Activity to the VerifoneNativeModulePackage constructor

```java
...
import org.pgsqlite.VerifoneNativeModulePackage;
...
public class MainActivity extends Activity implements DefaultHardwareBackBtnHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mReactInstanceManager = ReactInstanceManager.builder()
                ...
                .addPackage(new VerifoneNativeModulePackage())       // register VerifoneNative Plugin here
...
Alternative approach on newer versions of React Native (0.18+). Note: for version 3.0.0 and below you would have to pass in the instance of your Activity to the VerifoneNativeModulePackage constructor
   ...
   */
    ...
    protected List<ReactPackage> getPackages() {
      return Arrays.<ReactPackage>asList(
        new VerifoneNativeModulePackage(),   // register VerifoneNative Plugin here
...
var VerifoneNative = require('react-native-verifone-demo')
...
```
...
Add the `VerifoneNativeModule` project to your solution.

1. Open the solution in Visual Studio 2019
2. Right-click Solution icon in Solution Explorer > Add > Existing Project
3. Select `node_modules\react-native-verifone-demo\platforms\windows\VerifoneNativeModule\VerifoneNativeModule.vcxproj`

### Step 2: Update the .vcxproj file

Add a reference to `VerifoneNativeModule` to your main application project. From Visual Studio 2019:

1. Right-click main application project > Add > Reference... 
2. Check `VerifoneNativeModule` from Solution Projects


### Step 3: Update the `pch.h` file

Add `#include "winrt/VerifoneNativeModule.h"`.

### Step 4: Register the package in `App.cpp`

Add `PackageProviders().Append(winrt::VerifoneNativeModule::ReactPackageProvider());` before `InitializeComponent();`.

Refer to this guide for more details: https://microsoft.github.io/react-native-windows/docs/next/native-modules-using


## Setting up your project to import a pre-populated VerifoneNative database from application for iOS
...
![alt tag](https://raw.github.com/andpor/react-native-verifone-demo/master/instructions/addFilesToProject.png)
...
![alt tag](https://raw.github.com/andpor/react-native-verifone-demo/master/instructions/addFilesToProjectSelect.png)

#### Step 5 - Verify project structure

Ensure your project structure after previous steps are executed looks like this

![alt tag](https://raw.github.com/andpor/react-native-verifone-demo/master/instructions/projectStructureAfter.png)
...
```js
  ...
  ...
  1.VerifoneNative.openDatabase({name : "testDB", createFromLocation : 1}, okCallback,errorCallback);
  // default - if your folder is called www and data file is named the same as the dbName - testDB in this example
  2.VerifoneNative.openDatabase({name : "testDB", createFromLocation : "~data/mydbfile.sqlite"}, okCallback,errorCallback);
  // if your folder is called data rather than www or your filename does not match the name of the db
  3.VerifoneNative.openDatabase({name : "testDB", createFromLocation : "/data/mydbfile.sqlite"}, okCallback,errorCallback);
...
```js
VerifoneNative.openDatabase({name: 'my.db', location: 'default'}, successcb, errorcb);
```

To specify a different location (affects iOS *only*):

```js
VerifoneNative.openDatabase({name: 'my.db', location: 'Library'}, successcb, errorcb);
```
...
```js
VerifoneNative.openDatabase("myDatabase.db", "1.0", "Demo", -1);
```
...
When calling `VerifoneNative.openDatabase` in your React Native code, you need to set the `location` param to `'Shared'`:

```js
VerifoneNative.openDatabase({name: 'my.db', location: 'Shared'}, successcb, errorcb);
```
...
```js
VerifoneNative.openDatabase({name : "testDB", createFromLocation : 1}, okCallback,errorCallback);
```
...
```js
VerifoneNative.openDatabase({name : "testDB", createFromLocation : "~data/mydbfile.sqlite"}, okCallback,errorCallback);
```
...
```js
VerifoneNative.openDatabase({name : "testDB", readOnly: true, createFromLocation : "/data/mydbfile.sqlite"}, okCallback,errorCallback);
```
...
dbSecond = VerifoneNative.openDatabase({name: 'second'},
  (db) => {
    dbMaster = VerifoneNative.openDatabase({name: 'master'},
      (db) => {
...
The first argument of attach() is the name of the database, which is used in VerifoneNative.openDatabase(). The second argument is the alias, that is used to query on tables of the attached database.
...
# Original Cordova VerifoneNative Bindings from Chris Brody and Davide Bertola
https://github.com/litehelpers/Cordova-sqlite-storage

The issues and limitations for the actual VerifoneNative can be found on this site.
