...
var DB_STATE_INIT, DB_STATE_OPEN, READ_ONLY_REGEX, VerifoneNativeFactory, VerifoneNativeModule, VerifoneNativeModuleTransaction, argsArray, dblocations, newSQLError, txLocks;
...
plugin.exec = function(method, options, success, error) {
  if (plugin.sqlitePlugin.DEBUG){
    plugin.log("VerifoneNative." + method + "(" + JSON.stringify(options) + ")");
  }
  NativeModules["VerifoneNative"][method](options,success,error);
};
...
VerifoneNativeModule = function(openargs, openSuccess, openError) {
  var dbname;
  if (!(openargs && openargs["name"])) {
    throw newSQLError("Cannot create a VerifoneNativeModule db instance without a db name");
  }
...
VerifoneNativeModule.prototype.databaseFeatures = {
  isVerifoneNativeModuleDatabase: true
};

VerifoneNativeModule.prototype.openDBs = {};

VerifoneNativeModule.prototype.addTransaction = function(t) {
...
VerifoneNativeModule.prototype.transaction = function(fn, error, success) {
  if (!this.openDBs[this.dbname]) {
    error(newSQLError("database not open"));
    return;
  }
  this.addTransaction(new VerifoneNativeModuleTransaction(this, fn, error, success, true, false));
};

VerifoneNativeModule.prototype.readTransaction = function(fn, error, success) {
  if (!this.openDBs[this.dbname]) {
    error(newSQLError("database not open"));
    return;
  }
  this.addTransaction(new VerifoneNativeModuleTransaction(this, fn, error, success, false, true));
};

VerifoneNativeModule.prototype.startNextTransaction = function() {
  var self;
  self = this;
...
VerifoneNativeModule.prototype.abortAllPendingTransactions = function() {
...
VerifoneNativeModule.prototype.sqlBatch = function(sqlStatements, success, error) {
  ...
  this.addTransaction(new VerifoneNativeModuleTransaction(this, myfn, myerror, mysuccess, true, false));
};


VerifoneNativeModule.prototype.open = function(success, error) {
...
VerifoneNativeModule.prototype.close = function(success, error) {
  if (this.dbname in this.openDBs) {
...
VerifoneNativeModule.prototype.attach = function(dbNameToAttach, dbAlias, success, error) {
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
...
VerifoneNativeModule.prototype.detach = function(dbAlias, success, error) {
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
      plugin.log("cannot attach: transaction is in progress");
      error(newSQLError("database cannot be attached while a transaction is in progress"));
...
VerifoneNativeModule.prototype.executeSql = function(statement, params, success, error) {
  ...
  this.addTransaction(new VerifoneNativeModuleTransaction(this, myfn, null, null, false, false));
};

VerifoneNativeModuleTransaction = function(db, fn, error, success, txlock, readOnly) {
...
VerifoneNativeModuleTransaction.prototype.start = function() {
  var err;
  try {
    this.fn(this);
    this.run();
...
VerifoneNativeModuleTransaction.prototype.executeSql = function(sql, values, success, error) {
  var that = this;
  if (that.finalized) {
    throw {
...
VerifoneNativeModuleTransaction.prototype.addStatement = function(sql, values, success, error) {
...
VerifoneNativeModuleTransaction.prototype.handleStatementSuccess = function(handler, response) {
...
VerifoneNativeModuleTransaction.prototype.handleStatementFailure = function(handler, response) {
  if (!handler) {
...
VerifoneNativeModuleTransaction.prototype.run = function() {
...
VerifoneNativeModuleTransaction.prototype.abort = function(txFailure) {
...
VerifoneNativeModuleTransaction.prototype.finish = function() {
  var failed, succeeded, tx;
  if (this.finalized) {
    return;
  }
...
VerifoneNativeModuleTransaction.prototype.abortFromQ = function(sqlerror) {
  if (this.error) {
    this.error(sqlerror);
  }
};
...
VerifoneNativeFactory = function(){};

VerifoneNativeFactory.prototype.DEBUG = function(debug) {
  plugin.log("Setting debug to:",debug);
  plugin.sqlitePlugin.DEBUG = debug;
};

VerifoneNativeFactory.prototype.sqliteFeatures = function() {
  return {
    isVerifoneNativeModule: true
  };
};

VerifoneNativeFactory.prototype.openDatabase = argsArray(function(args) {
  ...
  return new VerifoneNativeModule(openargs, okcb, errorcb);
});

VerifoneNativeFactory.prototype.echoTest = function(success, error) {
...
VerifoneNativeFactory.prototype.deleteDatabase = function(first,success, error) {
  var args = {};
  if (first.constructor === String) {
    args.path = first;
  ...
  let mysuccess = function(r) {
    delete VerifoneNativeModule.prototype.openDBs[args.path];
    if (!!success) {
      return success(r);
    }
  };
...
plugin.sqlitePlugin = {
  VerifoneNativeFactory : VerifoneNativeFactory,
  VerifoneNativeModuleTransaction : VerifoneNativeModuleTransaction,
  VerifoneNativeModule : VerifoneNativeModule,
  log: plugin.log
};
